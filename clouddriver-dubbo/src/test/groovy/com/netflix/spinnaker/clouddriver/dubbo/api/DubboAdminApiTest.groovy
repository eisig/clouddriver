...
class DubboAdminApiTest extends Specification {
  ...
  static def apiEndpoint = "http://internal-app-health-test4-1916659500.cn-north-1.elb.amazonaws.com.cn:8080/dubbo-admin-api"
...
//
  ...
  def "disable service by instance id"() {
    setup:
    def instanceId = "i-0327f02629f25cbb2"
    when:
    def updateResult = dubboAdminApi.disableServicesByInstanceId(instanceId)
    then:
    updateResult.code == '200'
    def otherProvider = updateResult.updateProviders.find { (!it.enabled) }
    otherProvider == null
